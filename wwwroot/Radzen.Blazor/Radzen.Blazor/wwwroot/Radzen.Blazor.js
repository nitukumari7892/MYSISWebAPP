...
window.Radzen = {
  ...
  openPopup: function (parent, id, syncWidth, position, x, y, instance, callback, closeOnDocumentClick = true) {
    ...
    if (smartPosition && left + rect.width > window.innerWidth && window.innerWidth > rect.width) {
      ...
      console.log("Open popup condition 3 " + top);
    if (smartPosition) {
      if (position) {
          top = top + 20;
  ...
  togglePopup: function (parent, id, syncWidth, instance, callback,position) {
    var popup = document.getElementById(id);
    if (!popup) return;
    if (popup.style.display == 'block') {
      Radzen.closePopup(id, instance, callback);
    } else {
      Radzen.openPopup(parent, id, syncWidth, position, null, null, instance, callback);
    }
  },
